<!DOCTYPE html>
<html>
  <head>
  <meta http-equiv="content-type" content="text/html; charset=utf-8">
  <link rel="alternate" type="application/rss+xml" title="rss 2.0" href="/feed/">
  <meta name="viewport" content="width=device-width"/>
  <title>请求的页面不存在</title><link href="<?php echo esc_url(get_template_directory_uri());?>/static/style.scss" type="text/css" rel="stylesheet"/>
  </head>
  <body>
    <div class="body404">
      <div class="info404">
        <header id="header404" class="clearfix">
          <div class="site-name404"><i>404</i>
          </div>
        </header>
        <section>
          <div class="title404">
            <p>每一个平凡的日常<br>都是连续发生中的奇迹</p>
          </div><a href="<?php echo home_url(); ?>" real="nofollow" class="index404">返回首页</a>
        </section>
        <footer id="footer404">© <?php echo date("Y"); ?> <a href="<?php echo home_url(); ?>" real="nofollow"><?php bloginfo('name'); ?></a></footer>
      </div>
    </div>
  </body>
</html>